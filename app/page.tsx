...
// 1. Metadatos de Next.js para SEO y Open Graph
export const metadata = {
  ...
  openGraph: {
    ...
    images: [
      {
        url: "/images/og-home.jpg", // Asegúrate de tener esta imagen en public/images/
        width: 1216,
        height: 630,
...
export default async function Page() {
  // 2. Carga de datos desde la carpeta content/ o API
  const posts = await getAllPosts();
  
  // Separamos los posts según tu lógica
  const featured = posts.slice(0, 3);
  const latest = posts.slice(0, 6); // O posts.slice(3, 9) si no quieres repetir los destacados

  const main = featured[0];
  const small = featured.slice(1);

  // 3. Schema.org JSON-LD para que Google entienda que es una web con buscador
  const jsonLd = {
  ...
  return (
    <main className="min-h-screen bg-gray-950">
      {/* Inyección de Datos Estructurados SEO */}
      <script
      ...
      <section className="relative flex min-h-125 flex-col justify-center overflow-hidden border-b border-white/8 bg-gray-950 sm:min-h-150">
        ...
        <div className="absolute inset-0 z-0">
          <Image
            ...
            priority // Crítico para el SEO (LCP)
            sizes="100vw"
          />
          {/* Overlay oscuro */}
